...
import { openMutualFundDialogReducer, refreshDataTable,callChangeReducer,cpReducer } from 'store/reducers/menu';
...
const stocks = [
    ...
    {
        value: 'VO',
        label: 'Vanguard Mid Cap Index Fund'
    },
    {
        value: 'AMRMX',
        label: 'American Funds American Mutual Fund Class A'
    },
    {
        value: '0P0000MLHH.BO',
        label: 'Axis Bluechip Fund Growth'
    },
    {
        value: '0P00005UYJ.BO',
        label: 'UTI Equity Fund Regular Plan Growth'
    },
    {
        value: '0P0000XW1B.BO',
        label: 'SBI Small Cap Fund Direct Plan Growth '
    },
    {
        value: '0P0001BB7J.BO',
        label: 'SBI Equity Hybrid Fund Direct Dividend Reinvestment '
    },
    {
        value: '0P0000XVUE.BO',
        label: 'Parag Parikh Long Term Equity Regular Growth'
    },
    {
        value: '0P0000XVUE.BO',
        label: 'Axis Midcap Fund Direct Plan Dividend Payout '
    },
    {
        value: '0P0000XUYC.BO',
        label: 'ICICI Prudential Liquid Fund Direct Plan Growth'
    },
    {
        value: '0P0001784G.BO',
        label: 'Tata Digital India Fund Direct Growth'
    },
    {
        value: '0P00005V6E.BO',
        label: 'Aditya Birla Sun Life Digital India Fund Growth '
    }
];
...
export default function FullScreenDialog() {
    ...
    var { openMutualFundDialog, loader, currentUserId } = menu;
    console.log(menu);
    console.log(openMutualFundDialog);
    ...
    const stockDataCollentionRef = collection(db, 'Mutualfund');
    ...
    useEffect(() => {
        setOpen(openMutualFundDialog);
    }, [openMutualFundDialog]);
    ...
    const options = {
        ...
        headers: {
            'X-RapidAPI-Key': '38486ba306msh592b60bd09dbc07p1d1ccdjsn44dcd213c4c2',
        ...
        if (stockName != '' && dateOfInvestment != '' && investedAmount != '') {
            ...
            await axios
                ...
                .then((response) => {
                    ...
                    setCurrentData(response.data.prices[1]);
                })
                .catch((err) => {
                    console.log(err);
                });
    ...
    const handleClose = () => {
        setOpen(false);
        dispatch(openMutualFundDialogReducer({ openMutualFundDialog: false }));
        ...
        <div>
            <Dialog fullScreen open={open} onClose={handleClose} TransitionComponent={Transition}>
                ...
                <List>
                    <Card>
                        <CardContent>
                            <h1 style={{ marginLeft: 20, marginBottom: 0 }}>MUTUAL FUND</h1>
                        </CardContent>
                        <CardContent>
                            <form noValidate autoComplete="off">
                                <div style={{ marginLeft: '5vw', marginBottom: '2vw', display: 'flex' }}>
                                    <div style={{ marginRight: 20 }}>
                                        <Typography style={{ marginBottom: 20 }}>Name of Mutual Fund</Typography>
                            ...
                            <Grid item xs={12}>
                                <Paper className={classes.paper}>
                                    ...
                                    <Box sx={{ display: 'flex' }}>
                                        {docId ? (
                                            loader ? (
                                                <Datatable stockData={stockData} finType={'Mutualfund'} lcDate={'Last Checked'} />
                                            ) : (
                                                <CircularProgress
                                                    size="1rem"
                                                    color="inherit"
